...
export default function ActivityThreadScreen({ route, navigation }: Props) {
  ...
  async function cargar(uidParam?: string | null) {
    ...
    // Traemos nombre/poster de cada título, grupo o lista recomendado en el
    // hilo — en paralelo (antes era secuencial, uno atrás del otro; con un
    // chat de muchas recomendaciones eso se sentía notoriamente lento).
    await Promise.all(
      data.map(async (m) => {
...
const styles = StyleSheet.create({
  ...
  recomendacionEtiqueta: { fontSize: 11.5, opacity: 0.75 },
  recomendacionTitulo: { fontSize: 15, fontWeight: "700", marginTop: 1 },
  recomendacionSub: { fontSize: 12.5, opacity: 0.75, marginTop: 2 },
