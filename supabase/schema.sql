...
-- ============================================================
-- Traducciones de título/sinopsis por idioma, COMPARTIDAS entre todos
-- los usuarios (a diferencia del caché en memoria del teléfono, que es
-- por persona). Antes, cada usuario le preguntaba a TMDB el nombre en
-- su idioma cada vez que veía un título por primera vez — con muchos
-- usuarios simultáneos, eso multiplica los pedidos a la clave
-- compartida de TMDB y puede pegarle a la app entera. Ahora, la primera
-- vez que alguien necesita el nombre/sinopsis de un título en un idioma
-- puntual, queda guardado acá; el resto de los usuarios lo lee directo
-- de la base, sin volver a pedírselo a TMDB.
--
-- Guarda la respuesta CRUDA de TMDB para ese idioma puntual (no una
-- traducción propia) — la decisión de cuál mostrar (por ejemplo, usar el
-- original en inglés si es-419 resultó ser el mismo texto que es-ES) se
-- sigue calculando en la app a partir de estas filas.
-- ============================================================
create table if not exists title_translations (
  item_type text not null check (item_type in ('series', 'movie')),
  tmdb_id integer not null,
  language text not null,
  name text,
  overview text,
  synced_at timestamptz not null default now(),
  primary key (item_type, tmdb_id, language)
);

alter table title_translations enable row level security;
drop policy if exists "title_translations_select_all" on title_translations;
create policy "title_translations_select_all" on title_translations for select using (true);
drop policy if exists "title_translations_write_auth" on title_translations;
create policy "title_translations_write_auth" on title_translations for insert to authenticated with check (true);
drop policy if exists "title_translations_update_auth" on title_translations;
create policy "title_translations_update_auth" on title_translations for update to authenticated using (true);
